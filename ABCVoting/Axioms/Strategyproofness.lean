...
-/
def is_i_variant (inst inst' : ABCInstance V C) (i : V) : Prop :=
...
-/
lemma is_i_variant_refl (inst : ABCInstance V C) (i : V) :
    inst.is_i_variant inst i := by
  refine ⟨rfl, rfl, rfl, fun _ _ _ => rfl⟩

/--
is_i_variant is symmetric.
-/
lemma is_i_variant_symm (inst inst' : ABCInstance V C) (i : V)
    (h : inst.is_i_variant inst' i) : inst'.is_i_variant inst i := by
  obtain ⟨hv, hc, hk, ha⟩ := h
  refine ⟨hv.symm, hc.symm, hk.symm, ?_⟩
  intro v hv' hne
  rw [← hv] at hv'
...
-/
def modify_approval (inst : ABCInstance V C) (i : V) (A' : Finset C)
    (hi : i ∈ inst.voters)
...
-/
lemma modify_approval_is_variant (inst : ABCInstance V C) (i : V) (A' : Finset C)
...
-/
lemma modify_approval_voter (inst : ABCInstance V C) (i : V) (A' : Finset C)
    (hi : i ∈ inst.voters) (hA'_sub : A' ⊆ inst.candidates)
    (hA'_nonempty : A'.Nonempty) (hA'_proper : A' ⊂ inst.candidates) :
...
-/
lemma modify_approval_other (inst : ABCInstance V C) (i : V) (A' : Finset C)
    (hi : i ∈ inst.voters) (hA'_sub : A' ⊆ inst.candidates)
    (hA'_nonempty : A'.Nonempty) (hA'_proper : A' ⊂ inst.candidates)
    (v : V) (hne : v ≠ i) :
...
-/
def ABCRule.SatisfiesResoluteStrategyproofness {k : ℕ} (f : ABCRule V C k) : Prop :=
  ∀ (inst inst' : ABCInstance V C) (i : V),
    i ∈ inst.voters →
    inst.is_i_variant inst' i →
    inst'.approves i ⊂ inst.approves i →  -- strict subset
    ∀ (hk : inst.k = k) (hk' : inst'.k = k) (hres : f.IsResolute),
      ¬((f.resolute_committee inst' hk' hres) ∩ inst.approves i ⊃
        (f.resolute_committee inst hk hres) ∩ inst.approves i)
...
-/
lemma ABCRule.strategyproof_not_strict_superset {k : ℕ} (f : ABCRule V C k)
    (hsp : f.SatisfiesResoluteStrategyproofness)
    (inst inst' : ABCInstance V C) (i : V)
    (hi : i ∈ inst.voters)
    (hvar : inst.is_i_variant inst' i)
    (hsub : inst'.approves i ⊂ inst.approves i)
    (hk : inst.k = k) (hk' : inst'.k = k)
    (hres : f.IsResolute) :
    ¬((f.resolute_committee inst' hk' hres) ∩ inst.approves i ⊃
      (f.resolute_committee inst hk hres) ∩ inst.approves i) :=
  hsp inst inst' i hi hvar hsub hk hk' hres
...
-/
lemma ABCRule.sp_violation {k : ℕ} (f : ABCRule V C k)
    (inst inst' : ABCInstance V C) (i : V)
    (hi : i ∈ inst.voters)
    (hvar : inst.is_i_variant inst' i)
    (hsub : inst'.approves i ⊂ inst.approves i)
    (hk : inst.k = k) (hk' : inst'.k = k)
    (hres : f.IsResolute)
    (hgain : (f.resolute_committee inst' hk' hres) ∩ inst.approves i ⊃
             (f.resolute_committee inst hk hres) ∩ inst.approves i) :
    ¬f.SatisfiesResoluteStrategyproofness :=
  fun hsp => hsp inst inst' i hi hvar hsub hk hk' hres hgain
...
-/
lemma ABCRule.sp_no_strict_improvement {k : ℕ} (f : ABCRule V C k)
    (hsp : f.SatisfiesResoluteStrategyproofness)
    (inst inst' : ABCInstance V C) (i : V)
    (hi : i ∈ inst.voters)
    (hvar : inst.is_i_variant inst' i)
...
-/
lemma ABCRule.sp_chain_constraint {k : ℕ} (f : ABCRule V C k)
    (hsp : f.SatisfiesResoluteStrategyproofness)
    (inst inst' : ABCInstance V C) (i : V)
    (hi : i ∈ inst.voters)
    (hvar : inst.is_i_variant inst' i)
    ...
    (hres : f.IsResolute)
    (W : Finset C)
    (hW : f.resolute_committee inst hk hres = W) :
    -- The manipulated outcome cannot give i strictly more approved candidates
    ¬((f.resolute_committee inst' hk' hres) ∩ inst.approves i ⊃ W ∩ inst.approves i) := by
  rw [← hW]
  exact hsp inst inst' i hi hvar hsub hk hk' hres
...
-/
lemma ABCRule.sp_preserves_committee_ne_strict {k : ℕ} (f : ABCRule V C k)
    (hsp : f.SatisfiesResoluteStrategyproofness)
    (hwf : f.IsWellFormed)
    (inst inst' : ABCInstance V C) (i : V)
    (hi : i ∈ inst.voters)
    ...
    (hW_ne : f.resolute_committee inst hk hres ≠ inst.approves i) :
    -- After manipulation, committee is still NOT equal to i's approval set
    f.resolute_committee inst' hk' hres ≠ inst.approves i := by
  intro hcontra
  -- Suppose f(inst') = inst.approves i = A_i
  -- Then f(inst') ∩ A_i = A_i (size k)

  have h1 : f.resolute_committee inst' hk' hres ∩ inst.approves i = inst.approves i := by
    rw [hcontra]
    exact Finset.inter_self _

  -- Since f(inst) ≠ A_i and both have size k, their intersection is a strict subset of A_i
  have h_W_size : (f.resolute_committee inst hk hres).card = k :=
    (hwf inst hk).2 _ (f.resolute_committee_mem inst hk hres) |>.1

  have h2 : f.resolute_committee inst hk hres ∩ inst.approves i ⊂ inst.approves i := by
    rw [Finset.ssubset_iff_subset_ne]
    constructor
    · exact Finset.inter_subset_right
    · intro heq
      -- If intersection equals A_i, then A_i ⊆ f(inst)
      have hsub_W : inst.approves i ⊆ f.resolute_committee inst hk hres := by
        intro x hx
        have := Finset.ext_iff.mp heq x
        simp only [Finset.mem_inter] at this
        exact (this.mpr hx).1
      -- Both have size k, so subset implies equality
      have h_card_le : (f.resolute_committee inst hk hres).card ≤ (inst.approves i).card := by
        rw [h_W_size, h_size]
      have h_eq : inst.approves i = f.resolute_committee inst hk hres :=
        Finset.subset_iff_eq_of_card_le h_card_le |>.mp hsub_W
      exact hW_ne h_eq.symm

  -- Now h1 and h2 give us that f(inst') ∩ A_i ⊃ f(inst) ∩ A_i
  have h3 : f.resolute_committee inst' hk' hres ∩ inst.approves i ⊃
            f.resolute_committee inst hk hres ∩ inst.approves i := by
    rw [h1]
    exact h2

  -- But this contradicts strategyproofness
  exact hsp inst inst' i hi hvar hsub hk hk' hres h3
